    ...
    @if ($isTailwind)
        <div class="rounded-md">
            ...
            @foreach($filter->getOptions() as $key => $value)
                <div class="flex items-center space-x-1" wire:key="{{ $tableName }}-filter-{{ $filter->getKey() }}-multiselect-{{ $key }}-@if($filter->hasCustomPosition()){{ $filter->getCustomPosition() }}@endif">
                    <input
                        type="checkbox"
